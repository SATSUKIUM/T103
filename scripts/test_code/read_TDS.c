...
#define TRUE  1
  ...
  /* comment out by mibe  fd = init_connect(hostname,81); */
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr,"read_TDS num ch(1-4) [out_filename] [1/0 (output format 1 for ascii )][hoatname/IP]\n");
  ...
  for (i = 0 ; i < num_event ; i++){
    ...
    for(thisch=0;thisch<n_channel;thisch++){
      ...
      if(outmode)
	sprintf(output_fname,"%s_%d_ch%d.csv",output_fname_head,i,thisch+1);
	
      else
	sprintf(output_fname,"%s_%d_all.tds",output_fname_head,i);
